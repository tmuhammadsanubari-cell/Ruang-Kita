...
// Komponen Card dipisahkan ke luar untuk performa render yang lebih baik
interface ReservationCardProps {
  reservation: Reservation;
  onAction: (reservation: Reservation) => void;
}

const ReservationCard = ({ reservation, onAction }: ReservationCardProps) => (
  <motion.div
...
export function ManageReservationsPage({ showSuccess, showError }: ManageReservationsPageProps) {
  ...
  const openActionDialog = (reservation: Reservation) => {
  ...
  return (
    <div className="min-h-screen bg-white pt-20">
            ...
            {pendingReservations.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {pendingReservations.map((res) => (
                  <ReservationCard key={res.id} reservation={res} onAction={openActionDialog} />
                ))}
              </div>
            )}
          </TabsContent>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{selectedReservation?.facilityName}</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-slate-50 rounded-lg space-y-2 text-sm">
              ...
              <div className="flex items-center gap-2 text-slate-600">
                <Calendar className="w-4 h-4" />
                <span>{selectedReservation?.date.toLocaleDateString('id-ID')}</span>
              </div>
              <div className="flex items-center gap-2 text-slate-600">
                <Clock className="w-4 h-4" />
            ...
            <div>
              <Label htmlFor="admin-note">Catatan (Wajib untuk penolakan)</Label>
              <Textarea
                id="admin-note"
                value={adminNote}
